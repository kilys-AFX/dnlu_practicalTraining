...
def prepare_data(df_behavior, df_orders, df_users=None, df_products=None):
    ...
    标签：是否购买（1=购买，0=未购买）
    """
    # 如果提供了商品数据，关联获取category信息
    if df_products is not None and "product_id" in df_behavior.columns and "product_id" in df_products.columns:
        df_behavior = df_behavior.merge(df_products[["product_id", "category"]], on="product_id", how="left")
    ...
    # 如果有category列，添加类目数统计
    if "category" in df_behavior.columns:
        agg_dict["category"] = "nunique"
    
    behavior_features = df_behavior.groupby("user_id").agg(agg_dict).reset_index()
    
    # 动态构建列名
    base_cols = ["user_id", "view_count", "cart_count", "favorite_count", "buy_count",
                 "total_duration", "avg_duration", "max_duration", "product_count"]
    
    if "category" in df_behavior.columns:
        base_cols.append("category_count")
    
    behavior_features.columns = base_cols
