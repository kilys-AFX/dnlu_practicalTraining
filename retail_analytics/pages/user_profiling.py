...
def show_clustering_page():
    ...
    <div class="page-header">
        <h1>K-Means 用户聚类</h1>
        <div class="breadcrumb">首页 / <span>用户画像</span> / 用户聚类</div>
    </div>
    ...
    with st.container(border=True):
        ...
        if st.button("🚀 执行 K-Means 聚类", type="primary", use_container_width=True):
            with st.spinner("正在进行K-Means聚类分析..."):
                ...
                st.success(f"✅ 聚类完成！已将用户分为 {n_clusters} 个群体")
            ...
            with st.container(border=True):
                ...
                for i in range(n_clusters):
                    ...
                    fig2.add_trace(go.Scatterpolar(
                        r=values, theta=features + [features[0]],
                        fill='toself', name=f'聚类 {i}',
...
def show_lifecycle_page():
    ...
    <div class="page-header">
        <h1>生命周期管理</h1>
        <div class="breadcrumb">首页 / <span>用户画像</span> / 生命周期</div>
    </div>
    """, unsafe_allow_html=True)
